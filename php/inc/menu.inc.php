...
<?php
    if (is_created_session()) {
        ...
        if (get_config('show_menu_language_select', false)) {
            ...
            $n = count($keys);
            for ($i = 0; $i < $n; $i++) {
                $selected = ($keys[$i] == get_session_value('language')) ? ' selected' : '';
                echo '<option' . $selected . ' value="' . $keys[$i] . '">' . $names[$i] . '</option>';
            }
            echo '</select> ';
        }
        echo " | ";

        // Dashboard link (only shown if configured)
...
<div id="navManageItems" class="hidden">
	<ul>
		...
		<li><a href="manage_torns.php">Torns</a></li>
		<?php } ?>
