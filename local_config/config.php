...
class configuration_vars {
  ...
  ****************************************************************************/
   ...
   * and orders.
   */
  public $admin_email = "info@lavinagreta.org";
   ...
   *
   */
   public $email_SMTP_host = 'smtp.pangea.org';       // Set to use a SMTP server.
  //                                              // Use SMTP_host='debug' to don't send and dump messages to /local_config/debug_mail/
   public $email_SMTP_port = 587;                // Default = 25
   public $email_SMTP_user = 'lavinagreta'; // Default = $admin_email (is recommend to not set, so use default as $admin_email)
   public $email_SMTP_pswd = 'mypassword';      // If is not set or null SMTP_user/SMTP_pswd are not used.
   public $email_SMTP_encryption = 'tls';        // Valid values: 'ssl', 'tls', null. Default = null
...
public function __construct()
{
    ...
    if ($host === 'localhost' || $host === '127.0.0.1') {
        ...
        $this->db_name = 'aixada';
        $this->db_user = 'aixada';
        $this->db_password = 'aixada';
        $this->internet_connection = false;

    // STAGING
    } elseif (strpos($host, 'proves.lavinagreta.org') !== false) {

        $this->db_host = 'localhost';
        $this->db_name = 'aixada_proves';
        $this->db_user = 'lavinagreta-aixada';
        $this->db_password = 'Mvc3%h&kdfg924';
        $this->internet_connection = true;

    // PRODUCCIÓ
    } elseif (strpos($host, 'lavinagreta.org') !== false) {

        $this->db_host = 'localhost';
        $this->db_name = 'lavinagreta-aixada';
        $this->db_user = 'lavinagreta-aixada';
        $this->db_password = 'Mvc3%h&kdfg924';
        $this->internet_connection = true;
        ...
        // fallback segur
        die('Entorn desconegut. Revisa configuració.');
    }
}
